...
async def scan_bulk_receipts(
    ...
    if len(files) > MAX_BULK:
        raise HTTPException(
            status_code=400,
            detail=f"Maximum {MAX_BULK} files per bulk upload.",
        )
    ...
    for file in files:
        if file.content_type not in ALLOWED_TYPES:
            work_items.append({
                "bytes": None, "content_type": file.content_type,
                "filename": file.filename, "page_label": file.filename,
        ...
        if file.content_type in PDF_TYPES:
            ...
            if len(pages) == 1:
                work_items.append({
                    "bytes": pages[0], "content_type": "application/pdf",
                    "filename": file.filename, "page_label": file.filename,
                    "prefailed": None,
                })
            else:
                for idx, page_bytes in enumerate(pages, start=1):
                    work_items.append({
                        "bytes": page_bytes, "content_type": PDF_TYPE,
                        "filename": file.filename,
            ...
            work_items.append({
                "bytes": raw, "content_type": file.content_type,
                "filename": file.filename, "page_label": file.filename,
                "prefailed": None,
            })
    ...
    async def _process(item: dict) -> dict:
        if item.get("prefailed"):
            return {
                "filename": item["page_label"], "original_file": item["filename"],
                "status": "failed", "expense_id": None, "expense_status": None,
                "error": item["prefailed"], "vendor_name": None,
                "total_amount": None, "category": None,
                "subcategory": None, "fraud_risk_score": None,
            }
        return await process_single_file(
            file_bytes=item["bytes"], content_type=item["content_type"],
            filename=item["filename"], user_id=current_user.id,
            db=db, page_label=item["page_label"],
        )
        ...
        "expenses": [
            {
                ...
                "owner_email":      e.owner_email if hasattr(e, "owner_email") else None,
                "status":           e.status,
...
def get_expense_summary(
    ...
    pending_count  = db.query(Expense).filter(
        Expense.user_id == current_user.id, Expense.status == "pending_verification"
    ).count()
    rejected_count = db.query(Expense).filter(
        Expense.user_id == current_user.id, Expense.status == "rejected"
    ).count()
...
def get_all_expenses(
    ...
    flagged     = sum(1 for e in expenses if e.requires_manual_review)
    ...
    return {
        ...
        "filters_applied": {
            ...
            "gstin":       gstin,       "requires_review": requires_review,
...
# ═══════════════════════════════════════════════════════════════════════════════
# APPROVE / REJECT  ← fixed: uses Gmail SMTP via email_service.py
# ═══════════════════════════════════════════════════════════════════════════════
...
def approve_expense(
    ...
    # Save email to DB for future use
    if owner_email and hasattr(expense, "owner_email") and not expense.owner_email:
    ...
    # ── Send email via Gmail SMTP (email_service.py) ─────────────────
    email_to   = owner_email or (expense.owner_email if hasattr(expense, "owner_email") else "") or ""
    email_sent = False

    print(f"DEBUG EMAIL TARGET: '{email_to}'")
    ...
    if email_to and "@" in email_to:
        try:
            ...
            print(f"DEBUG EMAIL RESULT: sent={email_sent}, error={result.get('error')}")
        except Exception as e:
            print(f"DEBUG EMAIL EXCEPTION: {str(e)}")
    else:
        print("DEBUG EMAIL SKIP: no valid email address provided")
...
def reject_expense(
    ...
    """Move an expense to rejected and send email via Gmail SMTP."""
    print(f"DEBUG REJECT: expense_id={expense_id}, owner_email='{owner_email}'")
...
def send_rejection_alert(
    ...
    """Send manual rejection alert to owner."""
    expense = db.query(Expense).filter(
        Expense.id      == expense_id,
        Expense.user_id == current_user.id,
    ).first()
...
def clear_all_expenses(
    ...
    deleted = db.query(Expense).filter(Expense.user_id == current_user.id).delete()
    db.commit()
    return {
        "status":  "success",
